...
6. 清除封禁
   ...
   - 清除封禁的接口：/auth/delete/{username}/{ip}
...
| 键                                 | 说明                                                         |
| ---------------------------------- | ------------------------------------------------------------ |
| verify_code_{手机号}               | 记录手机请求的随机验证码，3分钟后失效                        |
| verify_code_limit_{手机号}         | 记录下刚进行了验证码请求的手机号，1分钟后失效，用于防止同一个手机号在1分钟内进行多次验证码请求，其值永远为1 |
| verify_code_request_count_{手机号} | 记录一个手机号进行验证码请求的累计次数                       |

#### 风控信息

| 键                                | 说明                                                         |
| --------------------------------- | ------------------------------------------------------------ |
| {用户名}_login_cities             | 集合，存放用户进行过登录操作的所有城市位置                   |
| {用户名}_login_devices            | 集合，存放用户进行过登录操作的所有设备ID                     |
| {用户名}\_{IP}\_wrong\_pwd\_count | 存放某个IP尝试登录某个账号的密码错误次数                     |
| ip_black_list                     | 集合，存放被封禁的IP                                         |
| {IP}\_request\_count              | 记录IP在五秒内进行的请求次数，过期时间为五秒。若五秒之内请求次数超过10次，则判定为一次高频请求 |
| {IP}\_block\_count                | 记录IP在十分钟内累计的高频请求次数，过期时间为10分钟。若10分钟之内高频请求次数达到5次，则封禁IP |
